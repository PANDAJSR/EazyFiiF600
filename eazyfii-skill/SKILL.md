...
- [科目③结构说明](./subject3-structure.md) - 科目③圆环在两杆中间的结构特点
...
- [科目4示例](./subject4-example.md) - 过单圈（固定高度）科目的详细示例（含坐标、穿圈技巧、灯光参考）

## 注意事项

1. **先读约束文档**：写程序前，**必须**先读取 [eazyfii-constraints.md](./eazyfii-constraints.md)，了解工具调用方法、约束条件和编程规范
2. **先打开项目**：调用项目相关工具前，请确保 EazyFii 应用已启动，并且应用界面上已经打开了要操作的无人机项目
3. **先参考科目示例**：涉及到科目完成时，务必先参考对应科目的示例文档（科目1参考 [subject1-example.md](./subject1-example.md)，科目2参考 [subject2-example.md](./subject2-example.md)，科目4参考 [subject4-example.md](./subject4-example.md)），了解坐标设置、转向逻辑、延时要求等关键细节。注意示例仅供参考，实际坐标需根据现场情况调整
4. **复检很重要**：修改程序后必须复检，不能只改一次就结束
5. **索引从1开始**：PatchDroneProgram 的索引是 1-based
6. **字段必须是字符串**：所有积木字段值必须是字符串类型，如 `"120"` 而不是 `120`
7. **等待端口文件**：脚本会等待最多 10 秒让 Electron 主进程创建端口文件
